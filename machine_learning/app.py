...
def predict_by_customer_id(customer_id: int):
    try:
        ...
        # Handle case when customer data not found
        if input_df.empty:
        ...
        # [Added] Business logic: Calculate remaining days
        ...
        if remaining_days < 0:
            ...
            message = f"This customer is {abs(remaining_days)} days overdue for expected repurchase!"
        elif remaining_days <= 3:
            status = "High Probability to Buy Soon"
            message = f"This customer is expected to repurchase within approximately {remaining_days} days."
        else:
            status = "Stable"
