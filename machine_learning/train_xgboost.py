...
from sklearn.metrics import mean_absolute_error, r2_score # Changed evaluation metrics
...
def main():
    ...
    # 3. Feature selection (added newly created cycle features) ⭐️
    features = [
    ...
    # 5. Model definition and training (Changed to Regressor) ⭐️
    ...
    # 6. Prediction and evaluation (Changed evaluation metrics) ⭐️
    y_pred = model.predict(X_test)
    print("\n [Model Evaluation Results]")
    # Check error instead of classification report
    mae = mean_absolute_error(y_test, y_pred)
    print(f"Mean Absolute Error: {mae:.2f} days") # Average prediction error in days
    print(f"R2 Score: {r2_score(y_test, y_pred):.4f}")

    # 7. Check feature importance (Which data was important for cycle prediction?)
